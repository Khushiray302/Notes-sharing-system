...
def view_mynotes(request):
    ...
    sorted_note = sorted(note, key=lambda note_instance: note_instance.subject)
    d = {'notes':sorted_note}
...
def view_allnotes(request):
    ...
    user = User.objects.get(id=request.user.id)
    notes = Notes.objects.filter(user = user).order_by('user__username') 

    sorted_user = sorted(notes, key=lambda user: user.user.username)
...
def view_users(request):
    ...
    sorted_users = sorted(users, key=lambda user: user.user.username)

    
    # Display the users.
    d = {'users': sorted_users }
...
def delete_users(request,pid):
    ...
    #sorted_users = sorted(user, key=lambda user: user.user.username)
    user.delete()
...
def pending_notes(request):
    ...
    if request.GET.get('searchkey') is not None:
        ...
        notes = Notes.objects.filter(subject__icontains=key,status = "pending")
    else:
        #notes = Notes.objects.filter(status = "Accept")
        notes = Notes.objects.filter(status = "pending")
    sorted_notes = sorted(notes, key=lambda user: user.user.username)
    d = {'notes':sorted_notes}
   # d = {'notes':notes}
...
def accepted_notes(request):
    ...
    if request.GET.get('searchkey') is not None:
        key = request.GET.get('searchkey')
        notes = Notes.objects.filter(subject__icontains=key,status = "Accept")
    else:
        notes = Notes.objects.filter(status = "Accept")
    sorted_notes = sorted(notes, key=lambda user: user.user.username)
    d = {'notes':sorted_notes}
...
def rejected_notes(request):
    ...
    if request.GET.get('searchkey') is not None:
        key = request.GET.get('searchkey')
        notes = Notes.objects.filter(subject__icontains=key,status = "Reject")
    else:
...
def all_notes(request):
    ...
    if request.GET.get('searchkey') is not None:
        key = request.GET.get('searchkey')
        notes = Notes.objects.filter(subject__icontains=key)
    else:
